...
from chromeCacheExtractor.caches.simpleCacheFile import simpleCacheFileParser as scfp
from chromeCacheExtractor.caches.blockCacheFile import blockCacheFileParser as bcfp
from dataClasses.simpleCache import simpleCacheData as sc
                                ...
                                for key, entry in block_entries.items():
                                    if entry and entry.data_addrs:
                                        ...
                                        if stream_1_data:
                                            ...
                                            logging.error(f"Failed to extract data from stream 1 for key: {key}")
                                    else:
                                        logging.error(f"Block Entry for key: {key} is None or missing data_addrs")
                            else:
